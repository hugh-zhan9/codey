...
- status line 最前面显示当前账号别名和套餐，便于确认当前正在使用哪个账号
...
- status line 会把当前账号显示成 `alias (Plan)` 的前缀形式
...
- 修正快速输入后立刻回车时，消息发送前会短暂停顿一下的体感问题

### 实现逻辑

- TUI composer 内部有 `paste-burst` 检测，会短暂 hold 单个 ASCII 字符，避免误判为粘贴
- 本地定制在提交路径上增加了一个轻量 flush：
  - 如果当前只是“单个待刷新的首字符”
  - 则先把它补回 composer 文本
  - 再继续正常 submit
- 真正的 burst paste 逻辑不变，只修正“快速打字后立即回车”的提交手感
...
- 上游如果改了 Enter 提交逻辑、paste-burst 状态机或 composer 输入路径，这部分需要重新确认有没有回退成“回车前先顿一下”的问题
