...
use crate::app_event::ReloadedAccountState;
...
impl App {
    ...
    async fn try_handle_local_history_op(
        ...
        match op.view() {
            AppCommandView::Other(Op::AddToHistory { text }) => {
                ...
                tokio::spawn(async move {
                    if let Err(err) =
                        legacy_core::append_message_history_entry(&text, &thread_id, &config).await
                    {
                        tracing::warn!(
                            thread_id = %thread_id,
                            error = %err,
            ...
            AppCommandView::Other(Op::GetHistoryEntryRequest { offset, log_id }) => {
                ...
                tokio::spawn(async move {
                    let entry_opt = tokio::task::spawn_blocking(move || {
                        legacy_core::lookup_message_history_entry(log_id, offset, &config)
                    })
                    .await
                    .unwrap_or_else(|err| {
    ...
    async fn handle_event(
        ...
        match event {
            ...
            AppEvent::ReloadAccount => {
                self.reload_account(app_server);
            }
            AppEvent::AccountReloaded { result } => match result {
                Ok(state) => {
                    self.chat_widget.on_account_reloaded(state);
                }
                Err(err) => {
                    tracing::warn!("account/read failed during TUI auth reload: {err}");
                    self.chat_widget
            ...
            AppEvent::SetSkillEnabled { path, enabled } => {
                ...
                {
                    Ok(()) => {
                        match AbsolutePathBuf::try_from(path.clone()) {
                            Ok(skill_path) => {
                                self.chat_widget.update_skill_enabled(skill_path, enabled);
                            }
                            Err(err) => {
                                tracing::warn!(
                                    error = %err,
                                    path = %path.display(),
                                    "skipping in-memory skill state refresh for non-absolute path"
                                );
                            }
                        }
                        if let Err(err) = self.refresh_in_memory_config_from_disk().await {
    ...
    let (status_account_display, plan_type, has_chatgpt_account, message, hint) =
        match response.account {
            Some(Account::ApiKey {}) => (
                ...
                None,
                false,
                "Reloaded auth state. Current auth: api-key.".to_string(),
                None,
            ),
            Some(Account::Chatgpt { email, plan_type }) => (
                ...
                Some(plan_type),
                true,
                "Reloaded auth state. Current auth: chatgpt.".to_string(),
                Some(email),
            ),
            None => (
                None,
                None,
                false,
                "Reloaded auth state. No active credentials are configured.".to_string(),
                None,
            ),
        };

    let rate_limit_snapshots = if requires_openai_auth && has_chatgpt_account {
        match fetch_account_rate_limits(request_handle).await {
            Ok(snapshots) => snapshots,
            Err(err) => {
                tracing::warn!(
                    error = %err,
                    "account/rateLimits/read failed during TUI auth reload"
                );
                Vec::new()
            }
        }
