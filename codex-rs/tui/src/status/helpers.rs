...
pub(crate) async fn discover_agents_summary(config: &Config) -> color_eyre::Result<String> {
    let paths = discover_project_doc_paths(config, codex_exec_server::LOCAL_FS.as_ref()).await?;
    let paths = paths
        .into_iter()
