...
#[tokio::test]
async fn reload_command_requests_account_reload_and_clears_rate_limits() {
    let (mut chat, mut rx, mut op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    ...
    assert_matches!(rx.try_recv(), Ok(AppEvent::ReloadAccount));
    assert_no_submit_op(&mut op_rx);
}

#[tokio::test]
async fn reload_result_updates_account_state_and_shows_info_message() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;

    chat.on_account_reloaded(crate::app_event::ReloadedAccountState {
        ...
        message: "Reloaded auth state. Current auth: api-key.".to_string(),
        hint: None,
    });
    ...
    assert_eq!(chat.current_plan_type(), Some(PlanType::Plus));
    ...
    let rendered = drain_insert_history(&mut rx)
        ...
        .collect::<Vec<_>>()
        .join("");
    assert_chatwidget_snapshot!("slash_reload_info_message", rendered);
}
